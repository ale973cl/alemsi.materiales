...
export default function FleetInspectionCamera({vehicleId,plate,driverName,currentKm,assignmentId,mode,startKm}:Props){
 ...
 const [odometer,setOdometer]=useState(currentKm);const [odometerConfirmed,setOdometerConfirmed]=useState(false);const [dashboard,setDashboard]=useState<{fuel_level:string|null;rpm:number|null;warning_lights:string|null;notes:string|null}|null>(null);const [readingDashboard,setReadingDashboard]=useState(false);
 const [location,setLocation]=useState<{latitude:number;longitude:number;accuracy:number}|null>(null);const [locationStatus,setLocationStatus]=useState("Solicitando ubicación puntual…");
 ...
 useEffect(()=>{if(!navigator.geolocation){setLocationStatus("GPS no disponible en este navegador.");return}navigator.geolocation.getCurrentPosition(position=>{setLocation({latitude:position.coords.latitude,longitude:position.coords.longitude,accuracy:position.coords.accuracy});setLocationStatus("Ubicación puntual lista para guardar.")},()=>setLocationStatus("Ubicación no concedida; la operación continuará sin GPS."),{enableHighAccuracy:true,timeout:12000,maximumAge:0});return()=>streamRef.current?.getTracks().forEach(t=>t.stop())},[]);
 ...
 async function analyzeDashboard(file:File){setReadingDashboard(true);setDashboard(null);setOdometerConfirmed(false);try{const form=new FormData();form.set("file",file);const response=await fetch("/api/flota/read-dashboard",{method:"POST",body:form});const payload=await response.json();if(!response.ok)throw new Error(payload?.error||"No se pudo leer el tablero.");const proposed=payload?.data?.odometer_km;if(Number.isInteger(proposed)&&proposed>=0)setOdometer(proposed);setDashboard({fuel_level:payload?.data?.fuel_level??null,rpm:payload?.data?.rpm??null,warning_lights:payload?.data?.warning_lights??null,notes:payload?.data?.notes??null})}catch(e){setDashboard({fuel_level:null,rpm:null,warning_lights:"Revisar",notes:e instanceof Error?e.message:"Lectura no disponible."})}finally{setReadingDashboard(false)}}
 function capture(){const video=videoRef.current;if(active===null||!video||!video.videoWidth)return;const photoIndex=active;const canvas=document.createElement("canvas");canvas.width=video.videoWidth;canvas.height=video.videoHeight;const ctx=canvas.getContext("2d");if(!ctx)return;ctx.drawImage(video,0,0);canvas.toBlob(blob=>{if(!blob)return;const file=new File([blob],VIEWS[photoIndex].replaceAll(" ","-")+".jpg",{type:"image/jpeg"});setPhotos(prev=>prev.map((p,i)=>i===photoIndex?file:p));closeCamera();if(photoIndex===4)void analyzeDashboard(file)},"image/jpeg",.88)}
 ...
 return <form action={action} className="fleetInspectionForm"><input type="hidden" name="vehicle_id" value={vehicleId}/>{assignmentId&&<input type="hidden" name="assignment_id" value={assignmentId}/>}
  {location&&<><input type="hidden" name="latitude" value={location.latitude}/><input type="hidden" name="longitude" value={location.longitude}/><input type="hidden" name="accuracy_m" value={location.accuracy}/></>}
  <div className="fleetActionForm">{mode==="take"?<><label>Conductor<input value={driverName} readOnly/></label><label>RUT<input name="driver_rut" required placeholder="12.345.678-9"/></label><label>Kilometraje inicial<input name="start_km" type="number" min={currentKm} value={odometer} onChange={e=>{setOdometer(Number(e.target.value));setOdometerConfirmed(false)}} required/></label></>:<label>Kilometraje final<input name="end_km" type="number" min={startKm??currentKm} value={odometer} onChange={e=>{setOdometer(Number(e.target.value));setOdometerConfirmed(false)}} required/></label>}<label>Observación<textarea name="comment" placeholder="Sin observaciones"/></label></div>
  ...
  {photos[4]&&<div className="fleetDashboardRead"><strong>{readingDashboard?"Leyendo solo la fotografía Tablero…":"Propuesta de tablero"}</strong><span>Odómetro propuesto: {readingDashboard?"Leyendo…":odometer||"Revisar"} · Combustible: {dashboard?.fuel_level??"Revisar"} · RPM: {dashboard?.rpm??"Revisar"} · Testigos: {dashboard?.warning_lights??"Sin lectura concluyente"}.</span><span>{dashboard?.notes||"Confirma o corrige el kilometraje antes de guardar. El lector nunca reemplaza tu confirmación."}</span><button type="button" className="fleetBtn fleetBtnGhost" disabled={readingDashboard||!Number.isInteger(odometer)||odometer<(mode==="take"?currentKm:startKm??currentKm)} onClick={()=>setOdometerConfirmed(true)}>{odometerConfirmed?"✓ Kilometraje confirmado":"Confirmar kilometraje"}</button></div>}
  <p className="fleetLocationStatus">{locationStatus}</p>{error&&<p className="fleetCameraError">{error}</p>}<InspectionSubmit complete={complete} mode={mode} odometerConfirmed={odometerConfirmed} readingDashboard={readingDashboard}/>
...
function InspectionSubmit({complete,mode,odometerConfirmed,readingDashboard}:{complete:number;mode:"take"|"return";odometerConfirmed:boolean;readingDashboard:boolean}){const {pending}=useFormStatus();return <button className="fleetBtn fleetBtnPrimary fleetConfirmInspection" disabled={complete!==7||pending||readingDashboard||!odometerConfirmed}>{pending?"Guardando fotos, GPS y registro…":complete!==7?(mode==="take"?"Confirmar toma":"Confirmar devolución")+" · "+complete+"/7":!odometerConfirmed?"Confirma el kilometraje para continuar":mode==="take"?"Confirmar toma · 7/7":"Confirmar devolución · 7/7"}</button>}
