...
export default function FleetInspectionCamera({vehicleId,plate,driverName,currentKm,assignmentId,mode,startKm}:Props){
 ...
 const [endKm,setEndKm]=useState(mode==="return"?String(currentKm):""); const [fuelLevel,setFuelLevel]=useState("");
 ...
 const minReturnKm=startKm??currentKm; const returnKm=Number(endKm); const returnKmValid=mode!=="return"||(endKm.trim()!==""&&Number.isInteger(returnKm)&&returnKm>=minReturnKm); const returnReady=mode!=="return"||(returnKmValid&&fuelLevel!=="");
 ...
 return <form action={action} className="fleetInspectionForm"><input type="hidden" name="vehicle_id" value={vehicleId}/>{assignmentId&&<input type="hidden" name="assignment_id" value={assignmentId}/>}
  <div className="fleetActionForm">{mode==="take"?<><label>Conductor<input value={driverName} readOnly/></label><label>RUT<input name="driver_rut" required placeholder="12.345.678-9"/></label><label>Kilometraje inicial<input name="start_km" type="number" min={currentKm} defaultValue={currentKm} required/></label></>:<><label>Kilometraje final<input name="end_km" type="number" inputMode="numeric" min={minReturnKm} value={endKm} onChange={e=>setEndKm(e.target.value)} required/></label><label>Nivel de combustible<select name="return_fuel_level" value={fuelLevel} onChange={e=>setFuelLevel(e.target.value)} required><option value="">Seleccionar nivel</option><option value="Vacío">Vacío</option><option value="1/4">1/4</option><option value="1/2">1/2</option><option value="3/4">3/4</option><option value="Lleno">Lleno</option></select></label></>}<label>Observación<textarea name="comment" placeholder="Sin observaciones"/></label></div>
  ...
  {mode==="return"&&!returnKmValid&&endKm!==""&&<p className="fleetCameraError">El kilometraje final no puede ser menor a {minReturnKm.toLocaleString("es-CL")} km.</p>}{error&&<p className="fleetCameraError">{error}</p>}<button className="fleetBtn fleetBtnPrimary fleetConfirmInspection" disabled={complete!==7||!returnReady}>{mode==="take"?"Confirmar toma":"Confirmar devolución"} · {complete}/7</button>
