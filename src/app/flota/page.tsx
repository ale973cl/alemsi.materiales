 ...
 return <main className="fleetPage">
  ...
  <section className="fleetSection fleetGarage">
   ...
   <div className="fleetVehicleGrid">{vehicles.map((v:any)=><Link className="fleetVehicleCard fleetPhotoButton" key={v.id} href={"/flota/"+v.id} aria-label={"Abrir "+v.plate+" "+v.label}>
    <div className="fleetVehiclePhoto">{v.photoUrl==="/flota/fleet-profiles.webp"?<div className={"fleetProfileSprite fleetProfileSprite"+v.plate} role="img" aria-label={"Fotografía "+v.plate}/>:v.photoUrl?<img src={v.photoUrl} alt={"Vista frontal "+v.plate}/>:<div className="fleetPhotoPending"><span>FOTO DEL VEHÍCULO</span><small>Sin fotografía de perfil</small></div>}<span className={"fleetStatus fleetPhotoStatus "+(v.status==="Disponible"?"fleetStatusOk":v.status==="En uso"?"fleetStatusWarning":"fleetStatusCritical")}>{v.status}</span></div>
