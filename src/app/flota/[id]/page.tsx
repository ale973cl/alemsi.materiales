...
{view==="resumen"&&<><section className="fleetActionPanel fleetActionPanelCamera"><div><span className="fleetStepLabel">ACCIÓN PRINCIPAL</span><h2>{currentUse?"Devolver vehículo":"Tomar vehículo"}</h2><p>{currentUse?"Completa la inspección 7/7 de devolución. Las fotografías quedan asociadas a este uso.":"Toca cada figura para abrir la cámara. Alinea el vehículo con la guía y completa las 7 fotografías antes de confirmar."}</p></div><FleetInspectionCamera vehicleId={v.id} driverName={p.full_name||"Usuario"} currentKm={Number(v.current_km)} assignmentId={currentUse?.id} startKm={currentUse?Number(currentUse.start_km):undefined} mode={currentUse?"return":"take"}/></section><section className="fleetDetailGrid">
