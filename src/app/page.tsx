...
export default async function Page() {
  ...
  const [clients, installations, materials, openCampaigns, pendingSurveys, openPOs, partialReceipts, openDispatches, financePending, emailPending, campaigns, activeCampaigns, consolidatedSurveys, orders, dispatches, audit, materialCatalog, clientInstallations, users, userInstallationAccess] = await Promise.all([
    ...
    supabase.from("clients").select("id,legal_name,rut,business_center,contracts!inner(id,name,code,active,installations(id,name,address,region,city,commune,delivery_contact_name,delivery_email,delivery_phone,phone,general_email,active))").eq("active",true).eq("contracts.active",true).eq("contracts.installations.active",true).order("legal_name"),
