...
export default async function Page() {
  ...
  const [clients, installations, materials, openCampaigns, pendingSurveys, openPOs, partialReceipts, openDispatches, financePending, emailPending, campaigns, activeCampaigns, consolidatedSurveys, orders, dispatches, audit, materialCatalog, inactiveMaterials, clientInstallations, masterClientInstallations, users, userInstallationAccess] = await Promise.all([
    ...
    supabase.from("dispatches").select("id,guide_number,internal_number,status,observations,recipient_name,recipient_email,recipient_phone,recipient_rut,recipient_role,recipient_signature,created_at,prepared_at,ready_at,in_transit_at,dispatched_at,delivered_at,cancelled_at,cancellation_reason,email_status,storage_status,installations(id,name,address,region,city,commune,delivery_contact_name,delivery_email,delivery_phone,contracts(id,name,code,clients(id,legal_name,rut))),dispatch_lines(id,material_id,required_qty,delivered_qty,pending_qty,materials(id,family,name,supplier_code,presentation,unit))").order("created_at",{ascending:false}),
