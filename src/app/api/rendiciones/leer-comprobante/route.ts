...
export async function POST(req:Request){
 try{
  ...
  const response=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:"Bearer "+key,"Content-Type":"application/json","HTTP-Referer":process.env.NEXT_PUBLIC_SITE_URL||"https://alemsi.vercel.app","X-Title":"ALEMSI Rendiciones"},body:JSON.stringify({model:"openrouter/free",messages:[{role:"user",content:[{type:"text",text:"Lee este comprobante chileno de gasto. Devuelve SOLO JSON válido, sin markdown. No inventes datos: si no se ve con claridad usa null. Campos: expense_date en YYYY-MM-DD, document_type uno de Factura|Boleta|Transferencia|Otro, provider_name, provider_rut, document_number, description breve del gasto, presented_amount como número entero CLP. Para TOTAL usa el monto final pagado, no neto ni IVA."},{type:"image_url",image_url:{url:image}}]}],temperature:0,max_tokens:500})});
  const raw=await response.json();
  if(!response.ok)return NextResponse.json({error:"El servicio de lectura no respondió. Puedes completar manualmente."},{status:502});
