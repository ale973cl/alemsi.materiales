...
export async function GET(){
 ...
 const [rr,ss,mm,cc,oo,sr,dd]=await Promise.all([
  ...
  supabase.from("clients").select("id,legal_name,rut,active,contracts(id,name,code,start_date,end_date,net_budget,active,installations(id,name,region,city,commune,surface_m2,collaborator_count,active),contract_budget_allocations(id,installation_id,allocated_net))").eq("active",true).order("legal_name"),
  ...
  supabase.from("dispatches").select("id,installation_id,contract_id,campaign_id,guide_number,status,dispatched_at,delivered_at,created_at,dispatch_lines(id,material_id,required_qty,delivered_qty,pending_qty,materials(id,name,family,presentation,unit,current_net_price))").order("created_at",{ascending:false})
 ]);
 for(const x of [rr,ss,mm,cc,oo,sr,dd])if(x.error)return NextResponse.json({error:x.error.message},{status:500});
 return NextResponse.json({receipts:rr.data||[],suppliers:ss.data||[],materials:mm.data||[],clients:cc.data||[],orders:oo.data||[],supplyRuns:sr.data||[],dispatches:dd.data||[],role:profile.role});
}
