...
async function loadStructure(s:any){
 const {data:clients,error}=await s.from("clients").select("id,legal_name,rut,active,contracts(id,name,active,installations(id,name,address,region,city,commune,active))");if(error)throw error;return clients||[];
}
function resolve(row:Row,clients:any[]){
 ...
 let matches=rn?clients.filter((c:any)=>rutNorm(c.rut)===rn):clients.filter((c:any)=>norm(c.legal_name)===cn);
...
export async function confirmInstallationCsvRows(rows:Row[]){const {s,user,p}=await ctx();if(!Array.isArray(rows)||!rows.length)throw new Error("No hay filas para confirmar");let created=0,skipped=0,review=0;for(const row of rows){const clients=await loadStructure(s);const x:any=resolve(row,clients);if(x.status==="EXISTE"){skipped++;continue;}if(x.status!=="AGREGAR"){review++;continue;}const payload={contract_id:x.contract.id,name:clean(row.installation),region:clean(row.region)||null,city:clean(row.city)||null,commune:clean(row.commune)||null,address:clean(row.address)||null,collaborator_count:0,active:true};const {data,error}=await s.from("installations").insert(payload).select("id,name,contract_id,region,city,commune,address").single();if(error)throw error;created++;await s.from("activity_log").insert({actor_id:user.id,actor_name:p.full_name||p.email,module:"Clientes e instalaciones",action:"Importar instalación CSV",entity_table:"installations",entity_id:data.id,new_data:data});}revalidatePath("/");return{ok:true,created,skipped,review,total:rows.length};}
