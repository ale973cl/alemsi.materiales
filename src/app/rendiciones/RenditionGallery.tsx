...
export default function RenditionGallery({items}:{items:Item[]}){
 ...
 return <><div className="renditionGallery">{items.map(item=>{const image=item.mime.startsWith("image/");return image?<button key={item.id} type="button" className="renditionGalleryCard" onClick={()=>setActive(item)} aria-label={"Ampliar "+item.label}><img src={item.url} alt={item.label}/><span><b>{item.label}</b></span></button>:<a key={item.id} className="renditionGalleryCard renditionGalleryPdf" href={item.url} target="_blank" rel="noreferrer"><span className="renditionPdfMark">PDF</span><span><b>{item.label}</b></span></a>})}</div>{active&&<div className="renditionLightbox" role="dialog" aria-modal="true" aria-label={active.label} onClick={()=>setActive(null)}><button type="button" className="renditionLightboxClose" onClick={()=>setActive(null)} aria-label="Cerrar">×</button><img src={active.url} alt={active.label} onClick={e=>e.stopPropagation()}/><div className="renditionLightboxCaption">{active.label}</div></div>}</>;
}
