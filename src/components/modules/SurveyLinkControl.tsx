...
export default function SurveyLinkControl({campaignId,installationId,installationName}:{campaignId:string;installationId:string;installationName:string}){
  ...
  async function run(mode:"link"|"email"){
    ...
    try{const result=await createSurveyAccessLink({campaign_id:campaignId,installation_id:installationId,mode,email:email||null,valid_hours:72});setLink(result.link);setMessage(mode==="email"?`Correo encolado${result.email?` para ${result.email}`:""}. El link también queda disponible para copiar.`:"Acceso seguro generado. Válido por 72 horas.");}
