...
export default function SurveyLinkControl({campaignId,installationId,installationName}:{campaignId:string;installationId:string;installationName:string}){
  ...
  return <div className="surveyAccessControl" aria-label={`Acceso al conteo de ${installationName}`}><input className="surveyAccessEmail" type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Correo para acceso (opcional)" aria-label={`Correo para acceso a ${installationName}`}/><button type="button" disabled={busy} onClick={()=>run("link")}>{busy?"Generando...":"Generar acceso"}</button><button type="button" disabled={busy} onClick={()=>run("email")}>{busy?"Procesando...":"Enviar por correo"}</button>{link&&<button type="button" onClick={()=>copy(link)}>Copiar link</button>}{link&&<input className="surveyAccessLink" aria-label="Link de acceso generado" readOnly value={link} onFocus={e=>e.currentTarget.select()}/>} {message&&<small className="surveyAccessMessage">{message}</small>}</div>;
}
