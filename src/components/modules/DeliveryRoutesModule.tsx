...
export default function DeliveryRoutesModule({routes,dispatches,users,currentUser,role}:{routes:DeliveryRoute[];dispatches:RouteDispatch[];users:RouteUser[];currentUser:{id:string;full_name?:string|null};role:string}){
 ...
 return <section style={{marginBottom:18,border:"1px solid #c8dce8",borderRadius:14,overflow:"hidden",background:"#fff"}}>
  ...
  <div style={{display:"grid",gap:10,padding:14}}>{routes.map(r=>{const links=routeLinks(r);const prepPerson=person(r.preparation_assignee),deliveryPerson=person(r.delivery_assignee);return <article key={r.id} style={{border:"1px solid #c8dce8",borderRadius:12,padding:12}}><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"start",flexWrap:"wrap"}}><div><b style={{fontSize:16}}>{r.route_name}</b><br/><small>{r.planned_date} · {links.length} instalaciones · {r.status}</small><br/><small>Prepara: <b>{prepPerson?.full_name||"—"}</b> · Entrega: <b>{deliveryPerson?.full_name||"—"}</b></small></div><div style={{display:"flex",gap:7,flexWrap:"wrap"}}><Link className="btn" href={`/rutas/${r.id}/consolidado`} target="_blank">Ver consolidado</Link>{r.status==="Asignada"&&(role!=="Supervisora"||r.preparation_assignee_id===currentUser.id)&&<button onClick={async()=>{try{await confirmRoutePrepared(r.id);router.refresh()}catch(e:any){setMessage(e.message)}}}>Marcar preparada</button>}{r.status==="Preparada"&&(role!=="Supervisora"||r.delivery_assignee_id===currentUser.id)&&<button onClick={async()=>{try{await startDeliveryRoute(r.id);router.refresh()}catch(e:any){setMessage(e.message)}}}>Iniciar ruta</button>}</div></div><details style={{marginTop:8}}><summary>Ver instalaciones</summary><ol>{links.map((x:any)=><li key={x.id}>{inst(x.dispatches)?.name||"Instalación"}</li>)}</ol></details></article>})}{!routes.length&&<div className="empty"><b>Aún no hay rutas guardadas</b><span>Al crear una ruta, el consolidado quedará asociado permanentemente.</span></div>}</div>
 </section>;
}
