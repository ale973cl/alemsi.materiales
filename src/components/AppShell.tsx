...
const labels:Record<string,string>={inicio:"Inicio",clientes:"1. Clientes / Contratos",maestro:"2. Maestro",levantamiento:"3. Levantamiento",cotejo:"4. Cotejo / Diferencias",consolidado:"5. Consolidado / Abastecimiento",kits:"6. Kits",oc:"7. Órdenes de Compra",ingreso:"8. Ingreso Mercadería",guias:"9. Guías / Despachos",pendientes:"10. Pendientes",historico:"11. Histórico / Costos",gestion:"12. Gestión",respaldos:"13. Respaldos"};
export default function AppShell({tab,setTab,state,patch,profiles,children}:{tab:string;setTab:(x:string)=>void;state:MaterialState;patch:(x:Partial<MaterialState>)=>void;profiles:ClientProfile[];children:ReactNode}){
 ...
 return <div className="app"><header className="top"><div><div className="brand">ALEMSI Materiales</div><div className="sub">Sistema modular V0.5</div></div><select value={state.role} onChange={e=>patch({role:e.target.value as Role})}>{["Supervisora","Gerencia","Finanzas","Bodega","Admin","Admin Total"].map(r=><option key={r}>{r}</option>)}</select></header>
