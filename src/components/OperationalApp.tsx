...
export default function OperationalApp({profile,summary,campaigns,orders,dispatches,audit,materialCatalog,materialCatalogSourceCount,materialCatalogError,clientInstallations,users,userInstallationAccess}:Props){
  ...
  return <div className="portal">
    ...
    <main className="workspace"><div className="pageHead"><div><p className="eyebrow">ALEMSI MATERIALES</p><h1>{labels[tab]}</h1></div><span className="live">● Datos centralizados</span></div>
      ...
      {!['inicio','maestro-materiales','maestros','usuarios','campañas','oc','despacho','pendientes','correos','auditoria'].includes(tab)&&<ModuleInfo tab={tab} summary={summary}/>} 
    </main></div>
  </div>;
}
