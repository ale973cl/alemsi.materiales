...
type Props={profile:any;summary:Record<string,number>;campaigns:any[];orders:any[];dispatches:any[];audit:any[];materialCatalog:MaterialCatalogRow[];materialCatalogSourceCount:number;materialCatalogError?:string|null;clientInstallations:ClientInstallationGroup[]};
...
export default function OperationalApp({profile,summary,campaigns,orders,dispatches,audit,materialCatalog,materialCatalogSourceCount,materialCatalogError,clientInstallations}:Props){
  ...
  return <div className="portal">
    ...
    <main className="workspace"><div className="pageHead"><div><p className="eyebrow">ALEMSI MATERIALES</p><h1>{labels[tab]}</h1></div><span className="live">● Datos centralizados</span></div>
      ...
      {!['inicio','maestro-materiales','maestros','campañas','oc','despacho','pendientes','correos','auditoria'].includes(tab)&&<ModuleInfo tab={tab} summary={summary}/>} 
    </main></div>
  </div>;
}
